# Roadmap

Work is split into useful sessions, not a fixed quota of commits. Check off a milestone only after the implementation and relevant tests pass.

- [x] Session 1: scan a folder, show branch/dirty state/latest commit, support JSON, integration tests.
- [ ] Session 2: add `--dirty` and name filters, with CLI coverage and examples.
- [ ] Session 3: report staged, unstaged, and untracked file counts; handle renames and unusual filenames.
- [ ] Session 4: display upstream ahead/behind state without fetching or network requests.
- [ ] Session 5: add configurable scan depth and exclusions, with traversal limits.
- [ ] Session 6: polish terminal output and documentation; review edge cases and prepare v0.2.

After the first solid release, choose the next project from the parent folder's idea list.
